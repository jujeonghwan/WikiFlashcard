...
if ($_REQUEST["sorttype"] == "") {
    ...
    $query .= "where fc_cardset = '" . $_REQUEST["cs_id"] . "' ";
    $query .= "order by fc_order, fc_id ";

    $result = db_query($query);

    $db_fc_id_array = array();

    $i = 0;
    while ($row = db_fetch_array($result)) {
        $i++;

        $db_fc_id_array[$row["fc_id"]] = $i;
    }

    $option_array = $db_fc_id_array;
    $option = get_select_option("", $option_array, "");
    tp_set("option_fc_id", $option);
...
if ($fc_id == "") {
    ...
    // $fc_id = get_first_random_flashcard($cs_id);         // 첫번째 순서의 임의의 플래시카드를 구함
}
tp_set("fc_id", $fc_id);
...
// $next_fc_id = get_next_random_flashcard($fc_id, $cs_id); // 다음 순서의 임의의 플래시카드를 구함
